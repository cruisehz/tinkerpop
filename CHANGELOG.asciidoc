...
* Introduced `WebSocketChannelizerV2` which fixes a bug with multithreaded use of the same `Client` object.
* Deprecated `WebSocketChannelizer` and renamed to `WebSocketChannelizerV1`.
* Deprecated `NioChannelizer` for both the client and server as it is not particularly used.
* Fixed bug in configuration of `Channelizer` for the `Cluster` Java driver object.
